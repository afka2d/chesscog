#!/usr/bin/env python3
"""
Train improved piece classification model using Marshall data
Uses the same ResNet18 architecture as the current working model
"""
...
class MarshallPieceDataset(Dataset):
    """Dataset for Marshall piece classification training"""
...
class MarshallPieceTrainer:
    """Trainer for Marshall piece classification model"""
    ...
    def create_dataset(self):
        ...
        for image_name, annotation in self.valid_annotations.items():
            ...
            try:
                ...
                processed += 1
                if processed % 50 == 0:
                    logger.info(f"Processed {processed} images, created {len(dataset)} samples")
    ...
    def load_working_piece_model(self):
        """Load the current working piece classification model architecture"""
        try:
            ...
            logger.info("✅ Loaded ResNet18 architecture (same as current working model)")
            return model
            
        except Exception as e:
            logger.error(f"❌ Error loading working model architecture: {e}")
            return None
    ...
    def train_model(self):
        """Train the piece classification model"""
        ...
        # Create data loaders
        train_dataset = MarshallPieceDataset(train_data, self.get_transforms(is_training=True))
        val_dataset = MarshallPieceDataset(val_data, self.get_transforms(is_training=False))
        ...
        optimizer = optim.Adam(model.parameters(), lr=0.0001)  # Lower learning rate to prevent overfitting
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3)
        
        # Training parameters
        max_epochs = 20  # Limit to prevent overfitting
        ...
        for epoch in range(max_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                # Save best model
                model_path = self.output_dir / "piece_classification_marshall.pt"
...
def main():
    ...
    logger.info("🎯 Marshall Piece Classification Training")
    logger.info("=" * 50)
    
    # Initialize trainer
    trainer = MarshallPieceTrainer()
