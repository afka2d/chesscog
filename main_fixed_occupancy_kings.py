...
This version fixes the array ambiguity error.
"""
...
app = FastAPI(
    title="Chess Position Scanner API - Fixed Occupancy + Kings",
...
async def health_check():
    return JSONResponse(content={
        "status": "healthy - Fixed Occupancy + Kings",
...
async def recognize_chess_position_with_corners(
    ...
    try:
        ...
        # Convert occupancy to list of booleans, handling the array ambiguity error
        occupancy = []
        for i in range(64):
            try:
                # Handle both single values and arrays
                if hasattr(occupancy_result[i], '__len__') and len(occupancy_result[i]) > 1:
                    # If it's an array, check if any element is True
                    occupancy.append(bool(np.any(occupancy_result[i])))
                else:
                    # If it's a single value, convert directly
                    occupancy.append(bool(occupancy_result[i]))
            except (IndexError, TypeError):
                # Fallback: assume occupied if we can't determine
