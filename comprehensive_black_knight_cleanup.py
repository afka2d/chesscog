#!/usr/bin/env python3
"""
Comprehensive cleanup of the black_knight directory.
...
def main():
    """Comprehensive cleanup of black_knight directory."""
    print("🔧 COMPREHENSIVE BLACK KNIGHT DIRECTORY CLEANUP")
    print("=" * 60)
    print("Checking all pieces against their FENs...")
    print()
    
    black_knight_dir = "grey_background_dataset/pieces/test/black_knight"
    
    if not os.path.exists(black_knight_dir):
        print(f"❌ Directory not found: {black_knight_dir}")
        return
    
    # Get all pieces in black_knight directory
    pieces = [f for f in os.listdir(black_knight_dir) if f.endswith('.png')]
    print(f"📊 Found {len(pieces)} pieces in black_knight directory")
    print()
    ...
    for piece_file in to_remove:
        piece_path = os.path.join(black_knight_dir, piece_file)
    ...
    for piece_file, target_type in to_move.items():
        source_path = os.path.join(black_knight_dir, piece_file)
    ...
    # Final count
    remaining_pieces = [f for f in os.listdir(black_knight_dir) if f.endswith('.png')]
    print(f"   📊 Remaining in black_knight: {len(remaining_pieces)} pieces")
