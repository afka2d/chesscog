#!/usr/bin/env python3
"""
Comprehensive cleanup of the black_pawn directory.
...
def main():
    """Comprehensive cleanup of black_pawn directory."""
    print("🔧 COMPREHENSIVE BLACK PAWN DIRECTORY CLEANUP")
    print("=" * 60)
    print("Checking all pieces against their FENs...")
    print()
    
    black_pawn_dir = "grey_background_dataset/pieces/test/black_pawn"
    
    if not os.path.exists(black_pawn_dir):
        print(f"❌ Directory not found: {black_pawn_dir}")
        return
    
    # Get all pieces in black_pawn directory
    pieces = [f for f in os.listdir(black_pawn_dir) if f.endswith('.png')]
    print(f"📊 Found {len(pieces)} pieces in black_pawn directory")
    print()
    ...
    for piece_file in to_remove:
        piece_path = os.path.join(black_pawn_dir, piece_file)
    ...
    for piece_file, target_type in to_move.items():
        source_path = os.path.join(black_pawn_dir, piece_file)
    ...
    # Final count
    remaining_pieces = [f for f in os.listdir(black_pawn_dir) if f.endswith('.png')]
    print(f"   📊 Remaining in black_pawn: {len(remaining_pieces)} pieces")
