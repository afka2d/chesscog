#!/usr/bin/env python3
"""
Local Development API for testing improvements without affecting production.
This runs on port 8001 and includes additional debugging features.
"""
...
app = FastAPI(
    title="Chess Position Scanner API - Local Development",
    description="Local development API for testing improvements. Runs on port 8001.",
    version="1.0.0-dev"
)
...
async def startup_event():
    ...
    logger.info("📍 Running on port 8001 (separate from production)")
    ...
    logger.info("🎉 Local Development API startup completed successfully")

@app.get("/health")
async def health_check():
    return JSONResponse(content={
        "status": "healthy - Local Development API",
        "port": 8001,
        ...
        "classifier_type": "Local Development - Working Models",
...
async def debug_info():
    ...
    return JSONResponse(content={
        ...
        "model_paths": {
            "occupancy": "runs/occupancy_classifier/ResNet/ResNet.pt",
            "color": "models/color_classifier_simple.pt",
            "piece_type": "models/piece_classifier_simple.pt"
        },
        "labels": {
            "color": COLOR_LABELS,
...
async def recognize_chess_position_with_corners(
    ...
    """
    Recognize chess position from image with given corners.
    ...
    try:
        ...
        logger.info(f"🔧 Local Development API - Processing image")
        ...
        for rank in range(8):
            for file in range(8):
                ...
                is_occupied = prediction == 1 and confidence > 0.3  # 1 = occupied, 0 = empty (lowered threshold)
...
if __name__ == "__main__":
    print("🚀 Starting Local Development API on port 8001")
    print("📍 Production API runs on port 8000")
    print("🔧 This API includes additional debugging features")
