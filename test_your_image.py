...
def test_chess_image(image_path):
    """Test the chess position extractor on a specific image."""
    ...
    try:
        ...
        for key, img in debug_images.items():
            if isinstance(img, np.ndarray):
                ...
                filename = f"{image_name}_corners_{key}.png"
...
def main():
    """Main function to test chess image recognition."""
    
    # You can change this to the path of your chess board image
    # For example: "your_chess_image.jpg" or "path/to/your/image.png"
    ...
    print(f"Testing chess position extraction on: {image_path}")
    print("=" * 60)
    
    result = test_chess_image(image_path)
