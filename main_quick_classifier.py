#!/usr/bin/env python3
"""
Main API with quick-trained two-stage piece classifier.
Uses MobileNetV2 for fast, accurate piece classification.
"""
...
app = FastAPI(
    title="Chess Position Scanner API - Quick Two-Stage Classifier",
    description="API for recognizing chess positions with quick-trained two-stage piece classification.",
    version="1.0.0"
)
...
class QuickTwoStageClassifier:
    def __init__(self, color_model_path, piece_model_path):
        # Load models
        self.color_model = self.load_model(color_model_path, 2)
        self.piece_model = self.load_model(piece_model_path, 6)
...
async def startup_event():
    logger.info("Starting up Chess Position Scanner API with Quick Two-Stage Classifier...")
    ...
    # Load quick two-stage piece classifier
    logger.info("Loading quick two-stage piece classifier...")
    global color_model, piece_model
    color_model_path = Path("models/color_classifier_quick.pt")
    piece_model_path = Path("models/piece_type_classifier_quick.pt")
    
    if not color_model_path.exists() or not piece_model_path.exists():
        logger.error("Quick classifier models not found. Please run train_quick_robust_classifier.py first.")
        raise RuntimeError("Quick classifier models not found")
    
    piece_classifier = QuickTwoStageClassifier(color_model_path, piece_model_path)
    color_model = piece_classifier.color_model
    piece_model = piece_classifier.piece_model
    logger.info("Quick two-stage piece classifier loaded successfully")
...
async def health_check():
    return JSONResponse(content={
        "status": "healthy - Quick Two-Stage Classifier",
        ...
        "classifier_type": "Quick Two-Stage (MobileNetV2)",
...
async def recognize_chess_position_with_corners(
    ...
    try:
        ...
        logger.info("🔧 Using quick two-stage piece classification")
        ...
        # Classify pieces using quick two-stage approach
        logger.info("Classifying pieces using quick two-stage approach...")
        board = chess.Board()
        board.clear()
        
        piece_classifier = QuickTwoStageClassifier("models/color_classifier_quick.pt", "models/piece_type_classifier_quick.pt")
        ...
        for rank in range(8):
            for file in range(8):
                ...
                if is_occupied:
                    ...
                    # Predict piece using quick two-stage approach
                    piece, combined_conf, color_conf, piece_conf = piece_classifier.predict_piece(square)
                    
                    if combined_conf > 0.3:  # Lower confidence threshold for quick model
