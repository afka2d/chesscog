#!/usr/bin/env python3
"""
Train Combined Piece Classification Model
Uses both grey background dataset AND Marshall data for maximum generalization
"""
...
class CombinedPieceTrainer:
    ...
    def load_working_piece_model_architecture(self):
        """Load the working piece classification model architecture"""
        try:
            # Try to find the working piece classification model
            possible_paths = [
                "models/piece_classifier_simple.pt",
                "checkpoints/working_system_20250910_175322/piece_classifier_simple.pt"
            ]
            ...
            if model_path is None:
                logger.warning("⚠️ Working piece model not found, using ResNet18 as fallback")
                # Create new ResNet18 model as fallback
                model = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
                num_features = model.fc.in_features
                model.fc = nn.Linear(num_features, 6)
                logger.info("✅ Created ResNet18 model as fallback")
                return model
            
            # Load the model to get architecture
            loaded_data = torch.load(str(model_path), map_location='cpu', weights_only=False)
            ...
            if isinstance(loaded_data, dict) and 'state_dict' in loaded_data:
                ...
                num_features = model.fc.in_features
                model.fc = nn.Linear(num_features, 6)
                logger.info(f"✅ Loaded working piece model architecture from: {model_path}")
                return model
                ...
                # It's a full model
                model = loaded_data
                logger.info(f"✅ Loaded working piece model from: {model_path}")
                
                # Check if it's the right architecture
                if hasattr(model, 'fc') and model.fc.out_features == 12:
                    # This is a 12-class model (6 pieces × 2 colors), we need 6-class
                    logger.info("Converting from 12-class to 6-class model...")
                    num_features = model.fc.in_features
                    model.fc = nn.Linear(num_features, 6)
                    logger.info("✅ Converted to 6-class model")
                elif hasattr(model, 'fc') and model.fc.out_features == 6:
                    logger.info("✅ Model already has 6 classes")
                else:
                    logger.warning("⚠️ Unknown model architecture, proceeding anyway")
    ...
    def create_combined_dataset(self):
        """Create combined dataset from grey background and Marshall data"""
        ...
        # Combine datasets
        combined_data = grey_data + marshall_data
        
        logger.info(f"Combined dataset: {len(grey_data)} grey background + {len(marshall_data)} Marshall = {len(combined_data)} total")
    ...
    def train_model(self):
        ...
        # Load working model architecture
        model = self.load_working_piece_model_architecture()
        model = model.to(self.device)
        
        # Setup training
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=0.001)  # Higher learning rate for combined training
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5)
        
        # Training parameters
        max_epochs = 25  # More epochs for combined training
        best_val_loss = float('inf')
        patience = 8
        patience_counter = 0
        
        logger.info(f"Training for maximum {max_epochs} epochs with early stopping (patience={patience})")
        logger.info(f"Learning rate: 0.001 (higher for combined training)")
        ...
        for epoch in range(max_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                # Save best model
                model_path = self.output_dir / "combined_piece_classifier.pt"
...
def main():
    ...
    logger.info("🎯 Combined Piece Classification Training")
    logger.info("=" * 60)
    ...
    if model is not None:
        ...
        logger.info("Model saved as: models_marshall_improved/combined_piece_classifier.pt")
