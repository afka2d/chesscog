#!/usr/bin/env python3
"""
Comprehensive cleanup of the black_king directory.
...
def main():
    """Comprehensive cleanup of black_king directory."""
    print("🔧 COMPREHENSIVE BLACK KING DIRECTORY CLEANUP")
    print("=" * 60)
    print("Checking all pieces against their FENs...")
    print()
    
    black_king_dir = "grey_background_dataset/pieces/test/black_king"
    
    if not os.path.exists(black_king_dir):
        print(f"❌ Directory not found: {black_king_dir}")
        return
    
    # Get all pieces in black_king directory
    pieces = [f for f in os.listdir(black_king_dir) if f.endswith('.png')]
    print(f"📊 Found {len(pieces)} pieces in black_king directory")
    print()
    ...
    for piece_file in to_remove:
        piece_path = os.path.join(black_king_dir, piece_file)
    ...
    for piece_file, target_type in to_move.items():
        source_path = os.path.join(black_king_dir, piece_file)
    ...
    # Final count
    remaining_pieces = [f for f in os.listdir(black_king_dir) if f.endswith('.png')]
    print(f"   📊 Remaining in black_king: {len(remaining_pieces)} pieces")
