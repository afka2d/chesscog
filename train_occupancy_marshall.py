#!/usr/bin/env python3
"""
Train Marshall Improved Occupancy Model
Fine-tune existing working occupancy model with Marshall data
"""
...
def create_occupancy_dataset():
    ...
    for image_name, annotation in valid_annotations.items():
        ...
        try:
            ...
            squares, labels = extract_squares_with_labels(warped_board, fen)
...
def extract_squares_with_labels(warped_board, fen):
...
def load_working_occupancy_model():
    """Load the existing working occupancy model"""
    logger.info("Loading working occupancy model...")
    
    # Load the working model
    model_path = Path("runs/occupancy_classifier/ResNet/ResNet.pt")
    if not model_path.exists():
        raise FileNotFoundError(f"Working occupancy model not found at {model_path}")
    
    # Load the model architecture and weights
    model = torch.load(str(model_path), map_location='cpu', weights_only=False)
    model.eval()
    
    logger.info("✅ Working occupancy model loaded successfully")
    return model

def train_occupancy_model():
    """Train improved occupancy detection model"""
    logger.info("🚀 Starting Marshall Occupancy Model Training")
    logger.info("=" * 60)
    ...
    model = load_working_occupancy_model()
    ...
    for epoch in range(20):  # Reduced epochs for faster training
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), output_dir / "occupancy_marshall.pt")
