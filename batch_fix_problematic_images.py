#!/usr/bin/env python3
"""
Batch script to fix all remaining problematic images in the dataset.
This script will systematically process each problematic image to correct
corner coordinates and regenerate piece images.
"""
...
class ImageFixer:
    """Class to handle fixing individual images."""
...
def get_problematic_images():
    """Get list of all problematic images from the scan."""
    problematic_images = [
        # VAL set (highest priority - fewer images)
        ("IMG_4754", "val"),
    ...
    # Remove duplicates (same image in multiple sets)
    unique_images = {}
    for image_name, dataset_type in problematic_images:
        if image_name not in unique_images:
            unique_images[image_name] = dataset_type
        else:
            # If already exists, prefer the set with fewer total images
            if dataset_type == "test":
                unique_images[image_name] = dataset_type  # Prefer test set
            elif dataset_type == "val" and unique_images[image_name] == "train":
                unique_images[image_name] = dataset_type  # Prefer val over train
    
    return list(unique_images.items())

def main():
    """Main function to run batch fixing."""
    print("🔧 Batch Fix for Problematic Images")
    print("=" * 60)
    
    # Get list of problematic images
    problematic_images = get_problematic_images()
    
    print(f"📊 Found {len(problematic_images)} problematic images to fix:")
    for image_name, dataset_type in problematic_images:
        print(f"   - {image_name} ({dataset_type.upper()})")
    
    print(f"\n💡 This will fix the remaining problematic images systematically.")
    print(f"   Note: Some images from the scan were not found and are skipped.")
    ...
    for i, (image_name, dataset_type) in enumerate(problematic_images, 1):
        ...
        fixer = ImageFixer(image_name, dataset_type)
    ...
    if successful_fixes > 0:
        print(f"\n✅ Dataset quality significantly improved!")
        print(f"   - {successful_fixes} problematic images fixed")
        print(f"   - Piece classifier accuracy should improve")
        print(f"   - Training data is now more accurate")
    else:
        print(f"\n❌ No images were successfully fixed.")
    
    print(f"\n💡 Next steps:")
    print(f"   1. Retrain your piece classifier with the improved dataset")
    print(f"   2. Test the new model accuracy")
    print(f"   3. The occupancy classifier remains unchanged")
