...
def test_position_extractor():
    """Test the chess position extractor on the provided image."""
    ...
    # Since we don't have the actual image file, let's use one of the existing test images
    # You can replace this with the path to your actual image
    ...
    try:
        ...
        # Test full recognition with debug images
        print("\n=== FULL RECOGNITION ===")
        board, corners_final, debug_images_full = recognizer.predict_with_debug(img_rgb, chess.WHITE)
        ...
        for key, img in debug_images_full.items():
            if isinstance(img, np.ndarray):
                # Convert RGB to BGR for OpenCV
                img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                filename = f"position_test_{key}.png"
        ...
        return board, corners_final, debug_images_full
