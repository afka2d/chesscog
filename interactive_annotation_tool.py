#!/usr/bin/env python3
"""
Interactive Chess Annotation Tool with Visual Display
...
import matplotlib
matplotlib.use('TkAgg')  # Force TkAgg backend for macOS
import matplotlib.pyplot as plt
import matplotlib.patches as patches
from matplotlib.widgets import Button
import os

class InteractiveChessAnnotator:
    def __init__(self, image_dir, output_dir, chess_set="set2"):
        ...
        self.ax = None
        self.corner_points = []
        self.corner_plot = None
        self.polygon_plot = None
        
        # Load images from last 24 hours
        self.load_recent_images()
        
    def load_recent_images(self):
        """Load all images from the marshall photos folder"""
        print("🔍 Finding images in marshall photos folder...")
        
        # Find all image files
        image_extensions = ['*.JPG', '*.jpg', '*.jpeg', '*.JPEG', '*.png', '*.PNG', '*.HEIC', '*.heic']
        for ext in image_extensions:
            self.image_files.extend(self.image_dir.glob(ext))
        
        self.image_files = sorted(self.image_files)
        print(f"📁 Found {len(self.image_files)} images in marshall photos folder")
        
        if len(self.image_files) == 0:
            print("❌ No images found in marshall photos folder.")
            return False
        return True
    ...
    def setup_matplotlib(self):
        ...
        print("🖼️  Matplotlib window should now be visible on your screen")
    ...
    def display_image(self):
        ...
        print(f"🖼️  Displaying image: {self.current_image.shape}")
        ...
        if self.current_corners is not None:
            ...
            print("📍 No corners placed yet - click on the image to place corners")
        ...
        self.fig.canvas.draw()
        print("✅ Image displayed in matplotlib window")
    ...
    def draw_corners(self):
        ...
        for i, (corner, color, label) in enumerate(zip(corners, colors, labels)):
            x, y = corner
            self.ax.plot(x, y, 'o', color=color, markersize=10, markeredgecolor='white', markeredgewidth=2)
            self.ax.text(x + 10, y + 10, label, color=color, fontsize=12, fontweight='bold')
        
        # Draw quadrilateral
        if len(corners) == 4:
            corners_closed = np.vstack([corners, corners[0]])  # Close the polygon
            self.ax.plot(corners_closed[:, 0], corners_closed[:, 1], 'cyan', linewidth=3, alpha=0.7)
    ...
    def load_and_display_current(self):
        ...
        # Skip auto-detection for now due to API issues
    ...
    def run(self):
        ...
        self.load_and_display_current()
        print("🖼️  If you don't see a window, try:")
        print("   1. Check your Dock for a Python window")
        print("   2. Press Cmd+Tab to cycle through windows")
        print("   3. Look for a window titled 'Chess Annotation Tool'")
...
def main():
    ...
    annotator = InteractiveChessAnnotator(image_dir, output_dir, chess_set)
    annotator.run()
