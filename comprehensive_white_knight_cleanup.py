#!/usr/bin/env python3
"""
Comprehensive cleanup of white_knight directory - check all pieces against FEN.
"""
...
def main():
    ...
    if not os.path.exists(white_knight_dir):
        print(f"❌ Directory not found: {white_knight_dir}")
        return
    
    pieces = [f for f in os.listdir(white_knight_dir) if f.endswith('.png')]
    print(f"🔍 Checking {len(pieces)} pieces in white_knight directory...")
    ...
    print(f"\n✅ Comprehensive white_knight cleanup complete:")
    print(f"   - Confirmed {confirmed_count} correct white knights")
