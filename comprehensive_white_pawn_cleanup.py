#!/usr/bin/env python3
"""
Comprehensive cleanup of white_pawn directory - check all pieces against FEN.
"""
...
def main():
    ...
    if not os.path.exists(white_pawn_dir):
        print(f"❌ Directory not found: {white_pawn_dir}")
        return
    
    pieces = [f for f in os.listdir(white_pawn_dir) if f.endswith('.png')]
    print(f"🔍 Checking {len(pieces)} pieces in white_pawn directory...")
    ...
    print(f"\n✅ Comprehensive white_pawn cleanup complete:")
    print(f"   - Confirmed {confirmed_count} correct white pawns")
