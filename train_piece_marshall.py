#!/usr/bin/env python3
"""
Train Marshall Improved Piece Classification Model
Fine-tune existing working piece model with Marshall data
"""
...
def piece_to_class(piece):
    """Convert piece character to class index"""
    piece_map = {
        'K': 0, 'Q': 1, 'R': 2, 'B': 3, 'N': 4, 'P': 5,  # White pieces
        'k': 6, 'q': 7, 'r': 8, 'b': 9, 'n': 10, 'p': 11  # Black pieces
    }
    return piece_map.get(piece, 12)  # 12 for empty squares
...
def create_piece_dataset():
    ...
    for image_name, annotation in valid_annotations.items():
        ...
        try:
            ...
            squares, labels = extract_squares_with_piece_labels(warped_board, fen)
...
def extract_squares_with_piece_labels(warped_board, fen):
    """Extract 64 squares and create piece labels from FEN"""
    squares = []
    labels = []
    ...
    for rank in range(8):
        for file in range(8):
            ...
            if piece == '.':
                ...
                # Convert piece to class index
                label = piece_to_class(piece)
                labels.append(label)
    
    return squares, labels

def load_working_piece_model():
    """Load the existing working piece model"""
    logger.info("Loading working piece model...")
    
    # Load the working model architecture
    def _get_piece_model_architecture(num_classes):
    ...
    model = _get_piece_model_architecture(13)  # 13 classes: 6 white + 6 black + 1 empty
    
    # Load weights from working model
    model_path = Path("models/piece_classifier_simple.pt")
    if not model_path.exists():
        raise FileNotFoundError(f"Working piece model not found at {model_path}")
    
    model.load_state_dict(torch.load(str(model_path), map_location='cpu'))
    model.eval()
    
    logger.info("✅ Working piece model loaded successfully")
    return model

def train_piece_model():
    """Train improved piece classification model"""
    logger.info("🚀 Starting Marshall Piece Classification Model Training")
    logger.info("=" * 60)
    
    # Create dataset
    dataset = create_piece_dataset()
    if len(dataset) < 100:
    ...
    model = load_working_piece_model()
    ...
    for epoch in range(20):  # Reduced epochs for faster training
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), output_dir / "piece_classification_marshall.pt")
