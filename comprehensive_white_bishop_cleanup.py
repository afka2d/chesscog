#!/usr/bin/env python3
"""
Comprehensive cleanup of the white_bishop directory.
...
def main():
    """Comprehensive cleanup of white_bishop directory."""
    print("🔧 COMPREHENSIVE WHITE BISHOP DIRECTORY CLEANUP")
    print("=" * 60)
    print("Checking all pieces against their FENs...")
    print()
    
    white_bishop_dir = "grey_background_dataset/pieces/test/white_bishop"
    
    if not os.path.exists(white_bishop_dir):
        print(f"❌ Directory not found: {white_bishop_dir}")
        return
    
    # Get all pieces in white_bishop directory
    pieces = [f for f in os.listdir(white_bishop_dir) if f.endswith('.png')]
    print(f"📊 Found {len(pieces)} pieces in white_bishop directory")
    print()
    ...
    for piece_file in to_remove:
        piece_path = os.path.join(white_bishop_dir, piece_file)
    ...
    for piece_file, target_type in to_move.items():
        source_path = os.path.join(white_bishop_dir, piece_file)
    ...
    # Final count
    remaining_pieces = [f for f in os.listdir(white_bishop_dir) if f.endswith('.png')]
    print(f"   📊 Remaining in white_bishop: {len(remaining_pieces)} pieces")
