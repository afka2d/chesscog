#!/usr/bin/env python3
"""
Test script for the Enhanced Chess Position Scanner API
"""
import requests
import json
import base64
...
def test_enhanced_api_health(base_url="http://localhost:8000"):
...
def test_enhanced_recognition(image_path, base_url="http://localhost:8000"):
    """Test the enhanced recognition endpoint."""
    ...
    try:
        with open(image_path, 'rb') as f:
            ...
            data = {
                'color': 'white',
                'debug_image_width': '600',
                'debug_image_height': '400'
            }
            
            print(f"Testing enhanced recognition with {image_path}...")
            ...
            if response.status_code == 200:
                ...
                for key, img_data in debug_images.items():
                    if img_data:
                        ...
                        output_path = f"enhanced_debug_{key}.png"
...
def main():
    """Run all tests."""
    print("=== Enhanced Chess Position Scanner API Test ===\n")
    
    # Test health endpoint
    print("1. Testing health endpoint...")
    if not test_enhanced_api_health():
        print("Health check failed. Make sure the API is running.")
        return
    
    print("\n2. Testing enhanced recognition...")
    ...
    for img in test_images:
        if Path(img).exists():
            test_enhanced_recognition(img)
            break
    
    print("\n3. Testing corner detection...")
    for img in test_images:
        if Path(img).exists():
    ...
    print("Check the generated debug images to see the enhanced features!")
