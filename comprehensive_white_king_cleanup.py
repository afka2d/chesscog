#!/usr/bin/env python3
"""
Comprehensive cleanup of white_king directory - check all pieces against FEN.
"""
...
def main():
    ...
    if not os.path.exists(white_king_dir):
        print(f"❌ Directory not found: {white_king_dir}")
        return
    
    pieces = [f for f in os.listdir(white_king_dir) if f.endswith('.png')]
    print(f"🔍 Checking {len(pieces)} pieces in white_king directory...")
    ...
    print(f"\n✅ Comprehensive white_king cleanup complete:")
    print(f"   - Confirmed {confirmed_count} correct white kings")
