#!/usr/bin/env python3
"""
Comprehensive cleanup of the black_bishop directory.
...
def main():
    """Comprehensive cleanup of black_bishop directory."""
    print("🔧 COMPREHENSIVE BLACK BISHOP DIRECTORY CLEANUP")
    print("=" * 60)
    print("Checking all pieces against their FENs...")
    print()
    
    black_bishop_dir = "grey_background_dataset/pieces/test/black_bishop"
    
    if not os.path.exists(black_bishop_dir):
        print(f"❌ Directory not found: {black_bishop_dir}")
        return
    
    # Get all pieces in black_bishop directory
    pieces = [f for f in os.listdir(black_bishop_dir) if f.endswith('.png')]
    print(f"📊 Found {len(pieces)} pieces in black_bishop directory")
    print()
    ...
    for piece_file in to_remove:
        piece_path = os.path.join(black_bishop_dir, piece_file)
    ...
    for piece_file, target_type in to_move.items():
        source_path = os.path.join(black_bishop_dir, piece_file)
    ...
    # Final count
    remaining_pieces = [f for f in os.listdir(black_bishop_dir) if f.endswith('.png')]
    print(f"   📊 Remaining in black_bishop: {len(remaining_pieces)} pieces")
