#!/usr/bin/env python3
"""
Re-extract all pieces from IMG_4755 using the corrected FEN and corners.
...
def extract_and_show_pieces(img, corners, fen, image_name):
    ...
    try:
        ...
        output_dir = "re_extracted_IMG_4755"
...
def show_extracted_pieces(piece_images, piece_info, output_dir):
    ...
    # Hide empty subplots
    for i in range(n_pieces, rows * cols):
        row = i // cols
        col = i % cols
        axes[row, col].axis('off')
    
    plt.suptitle(f'All Extracted Pieces from IMG_4755\nTotal: {n_pieces} pieces', fontsize=14)
    plt.tight_layout()
    
    # Save the combined image
    combined_path = os.path.join(output_dir, "all_pieces_grid.png")
    plt.savefig(combined_path, dpi=150, bbox_inches='tight')
    print(f"💾 Combined pieces grid saved to: {combined_path}")
    
    # Show the plot
    plt.show()
    
    print(f"\n🔍 Review each piece image:")
    print(f"   1. Check if the piece type matches the label")
    print(f"   2. Verify the square coordinates are correct")
    print(f"   3. Confirm the color classification is right")
    print(f"   4. Look for the problematic e8 piece specifically")
...
def main():
    """Main function to re-extract pieces from IMG_4755."""
    print("🔄 Re-extracting pieces from IMG_4755 with corrected FEN and corners")
    print("=" * 70)
    ...
    try:
        ...
        pieces_extracted = extract_and_show_pieces(img, corners, fen, "IMG_4755")
        
        print(f"\n✅ Re-extraction complete!")
        print(f"♟️  Total pieces extracted: {pieces_extracted}")
        print(f"📁 All pieces saved to: re_extracted_IMG_4755/")
        print(f"📁 Combined grid: re_extracted_IMG_4755/all_pieces_grid.png")
        
        # Highlight the problematic square
        print(f"\n⚠️  Key piece to verify:")
        print(f"   Square e8: Should show a black king (k) according to FEN")
        print(f"   If this shows a pawn instead, the FEN needs correction")
