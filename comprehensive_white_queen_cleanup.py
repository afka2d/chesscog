#!/usr/bin/env python3
"""
Comprehensive cleanup of white_queen directory - check all pieces against FEN.
"""
...
def main():
    ...
    if not os.path.exists(white_queen_dir):
        print(f"❌ Directory not found: {white_queen_dir}")
        return
    
    pieces = [f for f in os.listdir(white_queen_dir) if f.endswith('.png')]
    print(f"🔍 Checking {len(pieces)} pieces in white_queen directory...")
    ...
    print(f"\n✅ Comprehensive white_queen cleanup complete:")
    print(f"   - Confirmed {confirmed_count} correct white queens")
