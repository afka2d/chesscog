#!/usr/bin/env python3
"""
Validate Combined Piece Classification Model
Test accuracy on both Marshall data and previous training data
"""
...
class CombinedPieceValidator:
    ...
    def load_previous_training_data(self):
        """Load previous piece classification training data from grey_background_dataset"""
        logger.info("Loading previous training data from grey_background_dataset...")
    ...
    def create_confusion_matrix(self, results, dataset_name):
        ...
        # Save plot
        output_path = f"confusion_matrix_{dataset_name.lower().replace(' ', '_')}.png"
    ...
    def run_validation(self):
        ...
        logger.info("🚀 Starting Combined Piece Classification Model Validation")
        logger.info("=" * 70)
        ...
        previous_results = self.evaluate_model(model, previous_data, "Previous Training Data")
        if previous_results:
            self.create_confusion_matrix(previous_results, "Previous Training Data")
        ...
        if previous_results:
            logger.info(f"Previous Data Accuracy: {previous_results['overall_accuracy']:.2f}%")
        else:
            logger.info("Previous Data Accuracy: N/A (no data)")
        
        # Per-class comparison
        if marshall_results and previous_results:
            logger.info("\n📋 Per-Class Accuracy Comparison:")
            logger.info("Piece Type | Marshall | Previous | Difference")
            logger.info("-" * 50)
            for i, piece_name in enumerate(self.piece_names):
                marshall_acc = marshall_results['class_accuracies'][i]
                previous_acc = previous_results['class_accuracies'][i]
                diff = marshall_acc - previous_acc
                logger.info(f"{piece_name:10} | {marshall_acc:8.2f}% | {previous_acc:8.2f}% | {diff:+7.2f}%")
