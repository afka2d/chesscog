#!/usr/bin/env python3
"""
Marshall Photos Chess Annotation Tool
Interactive tool for annotating chess images from the marshall photos folder
...
class MarshallChessAnnotator:
    ...
    def load_chess_images(self):
        """Load all chess images from marshall photos folder"""
        print("🔍 Finding chess images in marshall photos...")
        
        # Find all image files including HEIC
        image_extensions = ['*.HEIC', '*.heic', '*.JPG', '*.jpg', '*.jpeg', '*.JPEG', '*.png', '*.PNG']
        ...
        if len(self.image_files) == 0:
            print("❌ No chess images found. Please check the marshall photos folder.")
            return False
        return True
    
    def load_image(self, image_path):
        """Load and display an image, handling HEIC files"""
        try:
    ...
    def detect_corners_automatically(self):
        ...
        try:
            ...
            # Convert image back to BGR for API call
            bgr_image = cv2.cvtColor(self.current_image, cv2.COLOR_RGB2BGR)
            
            # Encode image as JPEG for API
            success, encoded_img = cv2.imencode('.jpg', bgr_image)
            if not success:
    ...
    def setup_matplotlib(self):
        ...
        self.fig.suptitle(f"Marshall Chess Annotation - Image {self.current_index + 1}/{len(self.image_files)}", fontsize=16)
    ...
    def run(self):
        ...
        print(f"\n🎯 MARSHALL CHESS ANNOTATION TOOL")
...
def main():
    ...
    annotator = MarshallChessAnnotator(image_dir, output_dir, chess_set)
    annotator.run()
