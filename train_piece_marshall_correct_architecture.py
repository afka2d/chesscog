#!/usr/bin/env python3
"""
Train Marshall piece classification model using the SAME architecture as the original model.
...
class MarshallPieceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        squares, labels = self.extract_squares_with_piece_labels(warped, fen)
...
def create_piece_model_architecture(num_classes):
...
def load_original_piece_model():
    """Load the original piece model to get the architecture"""
    try:
        ...
        model = create_piece_model_architecture(6)  # 6 piece types
...
def train_piece_model():
    """Train the Marshall piece classification model"""
    logger.info("🚀 Starting Marshall piece classification model training with correct architecture")
    ...
    original_model = load_original_piece_model()
    ...
    dataset = MarshallPieceDataset(annotations, "marshall_chess_annotations", transform)
    if len(dataset) == 0:
    ...
    for epoch in range(num_epochs):
        ...
        if num_batches > 0:
            ...
            if avg_loss < best_loss:
                ...
                torch.save(model.state_dict(), output_dir / "piece_marshall_correct_architecture.pt")
